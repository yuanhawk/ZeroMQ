...
namespace ChatZ.Client
{
  public static class Proxy
  {
    ...
    public static Task Start(Config config, CancellationToken token) 
      => Task.Run(() => 
      {
        ...
        { 
          ...
          subSock.Subscribe(new[] { GroupTopic, NewsFrame(config.Handle) });
    ...
    internal static bool Transmit(string input, string target = GroupSender)
    {
      try
      {
        return inbox.TryAdd((target, input));
      }
      catch (Exception ex)
      {
