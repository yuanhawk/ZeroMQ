...
namespace ChatZ.Client
{
  public class MainWindowViewModel : INotifyPropertyChanged
  {
    ...
    public MainWindowViewModel(Proxy.Config proxyConfig, CancellationToken token)
    {
      ...
      Proxy.NewsStream.ObserveOnDispatcher().Subscribe(UpdateChatHistory);
    ...
    public RelayCommand SendMessage
    {
      ...
      {
        var sent = (Filter == Lobby.UserHandle)
                 ? Proxy.Transmit(Input)
                 : Proxy.Transmit(Input, target: Filter);
        if (!sent)
        {
          MessageBox.Show("Please try again", "Unable to send message", MessageBoxButton.OK);
        }
        else
        {
          Input = string.Empty;
