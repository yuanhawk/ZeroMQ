...
{ // Extend server code to emit notification whenever user enters or leaves the chat
  public static class Program
  {
    public static void Main(string[] _args)
    {
      ...
      { 
        ...
        var pubSock = context.Pub(); // Context object to create new socket
        pubSock.Bind(publish); // binds as this is the stable node of the pubsub channel
        ...
        while (true)
        {
          ...
          foreach (var client in expired.ToList())
          {
            ...
            pubSock.SendAll(NewsMessage(GroupSender, $"Goodbye, {client}.")); // Publish a broadcast
